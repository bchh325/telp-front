      ...
      <PagerView
        ...
        style={{ flex: 1 }}>
        {
          sampleData.businesses.map((value, index: number) => {
            return (
              <View key={index + ":view"} style={classes}>
                <Image key={index + ":image"} style={styles.image} source={{ uri: value.image_url }} />
                <PlaceDetails
                  key={index + ":placedetails"}
                  placeName={value.name}
                  placeRating={value.rating}
                  reviewCount={value.review_count}
                />
                <ActionBar key={index + ":actionbar"} navigator={navigator} />
              </View>
            )
          })
        }
